...
class Model {
    ...
    init(name:String, shader: BaseEffect, vertices: [Vertex], indices: [GLubyte]) {
        ...
        glVertexAttribPointer(VertexAttributes.normal.rawValue, 3, GLenum(GL_FLOAT), GLboolean(GL_FALSE), GLsizei(MemoryLayout<Vertex>.size), BUFFER_OFFSET((3+4+2)*MemoryLayout<GLfloat>.size))
